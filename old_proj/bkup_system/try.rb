...
$:.unshift("/usr/lib/ruby/gems/1.8/gems/nokogiri-1.5.0/lib/")
...
class DateTime
	def to_time
		t = Time.mktime(self.year, self.month, self.day)
	end
end
...
Dir.mkdir "#{$airline}_calls" if not File.exists? "#{$airline}_calls"
	...
	if price.is_a? Symbol
		...
		case price
			...
			when :different_dates
				...
				record.departure_date += ONE_DAY; record.return_date += ONE_DAY
...
def write_row csv, row
	csv << row
	csv.instance_variable_get(:@dev).flush
	#csv.flush
end
...
if mode == "wb"
	...
	csv = CSV.open(csv_output_file, "wb")
	csv_existing_records.each do |row|
		write_row csv, row
	end
end
